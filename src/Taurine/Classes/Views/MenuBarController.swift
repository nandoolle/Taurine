...
class MenuBarController: NSObject {
    ...
    private func updateIcon() {
        ...
        if self.viewModel.needsRecovery {
            button.image = NSImage(systemSymbolName: "exclamationmark.triangle", accessibilityDescription: String(localized: "Sleep needs to be restored"))
        } else {
            button.image = self.viewModel.isActive ? self.activeIcon : self.inactiveIcon
        }
        // Keep the inactive appearance stable, including between background checks.
        button.appearsDisabled = false
        button.alphaValue = self.viewModel.isActive || self.viewModel.needsRecovery ? 1 : 0.65

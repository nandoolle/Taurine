...
sources=(src/Taurine/Classes/*.swift src/Taurine/Classes/*/*.swift)
"$compiler" -O -parse-as-library -swift-version 5 -default-isolation MainActor \
    -module-name Taurine -target "$(uname -m)-apple-macos14.6" -sdk "$sdk" \
    -module-cache-path "$SWIFT_MODULECACHE_PATH" \
    "${sources[@]}" -o .build/app/Taurine
...
/usr/libexec/PlistBuddy -c 'Add :CFBundleShortVersionString string 0.2.3' "$app/Contents/Info.plist"
/usr/libexec/PlistBuddy -c 'Add :CFBundleVersion string 5' "$app/Contents/Info.plist"
